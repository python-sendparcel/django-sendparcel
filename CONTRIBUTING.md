...
Thank you for considering a contribution to `django-sendparcel` — the Django
adapter for the sendparcel ecosystem.
...
1. Clone the repository and navigate to the `django-sendparcel` directory.
...
Tests use **pytest** with **pytest-django** and **pytest-asyncio**
(`asyncio_mode = "auto"`).
...
uv run --extra dev ruff format --check sendparcel_django tests
```
