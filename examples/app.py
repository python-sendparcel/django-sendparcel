"""Django single-page shipping demo with Tabler + HTMX."""
...
<html lang="en">
  <head>
    ...
    <title>Django Sendparcel Demo</title>
    <link
...
def _payload_from_request(request: HttpRequest) -> dict[str, str]:
    fields = [
        "provider",
        "recipient_email",
        "recipient_phone",
    ...
    return {field: str(request.POST.get(field, "")) for field in fields}


async def _create_paid_shipment(checkout: Checkout) -> None:
    order = DemoOrder(
        order_id=f"order-{checkout.checkout_id[:8]}",
        sender_email=checkout.sender_email,
        recipient_email=checkout.recipient_email,
        recipient_phone=checkout.recipient_phone,
        recipient_address=checkout.recipient_address,
        recipient_locker=checkout.recipient_locker,
        package_size=checkout.package_size,
    )
    shipment = await flow.create_shipment(order, checkout.provider)
    shipment = await flow.create_label(shipment)
    checkout.paid = True
    checkout.shipment_id = str(shipment.id)
    checkout.tracking_number = str(shipment.tracking_number)
    labels[checkout.checkout_id] = _build_pdf(
...
def checkout(request: HttpRequest) -> HttpResponse:
    ...
    if request.method != "POST":
        return HttpResponse(_render_error("Use POST for checkout."), status=405)

    payload = _payload_from_request(request)
    error = _validate_payload(payload)
    if error:
        return HttpResponse(_render_error(error), status=400)
    ...
    return HttpResponse(_render_payment(checkout_data))


def pay(request: HttpRequest, checkout_id: str) -> HttpResponse:
    """Simulate payment and create a PDF shipment label."""
    if request.method != "POST":
        return HttpResponse(_render_error("Use POST for payment."), status=405)

    checkout_data = checkouts.get(checkout_id)
    if checkout_data is None:
        return HttpResponse(_render_error("Unknown checkout."), status=404)

    if not checkout_data.paid:
        anyio.run(_create_paid_shipment, checkout_data)

    return HttpResponse(_render_label(checkout_data))


def label_pdf(request: HttpRequest, checkout_id: str) -> HttpResponse:
    ...
    response["Content-Disposition"] = (
        f'inline; filename="label-{checkout_id}.pdf"'
    )
    return response
...
urlpatterns = [
    ...
    path("pay/<str:checkout_id>", pay, name="pay"),
    path("label/<str:checkout_id>.pdf", label_pdf, name="label_pdf"),
]
